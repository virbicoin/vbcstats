...
const ChartCard: React.FC<ChartCardProps> = ({
    ...
    if (type === 'sparkline') {
      ...
      chart
        ...
        .on('mouseout', function () {
          ...
          // Reset circle
          d3.select(this).attr('r', 3).attr('fill', 'transparent').attr('stroke', 'transparent');
        });
  ...
  return (
    <div
      className="group relative chart-container bg-gradient-to-br from-[#0d1421] to-[#0a1018] rounded-xl border border-[#1e3a5f]/50 p-5 
    ...
    >
      ...
      <div className="relative z-10">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className={`text-sm font-bold ${className}`}>{getLastValue()}</span>
        </div>
        <svg ref={svgRef} width="200" height="60" className={`w-full h-15 ${className}`} />
      </div>
    </div>
  );
};
