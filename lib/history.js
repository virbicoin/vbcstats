...
History.prototype.add = function(block, id, trusted, addingHistory)
{
	...
	if( !_.isUndefined(block) && !_.isUndefined(block.number) && !_.isUndefined(block.uncles) && !_.isUndefined(block.transactions) && !_.isUndefined(block.difficulty) && block.number > 0 )
	{
		...
		// Fix corrupted/truncated timestamps from VirBiCoin API
		if (block.timestamp && block.timestamp < 1e6) {
			console.log('Fixing corrupted timestamp for block', block.number, '- timestamp was:', block.timestamp);
			block.timestamp = Math.floor(now / 1000); // Convert to seconds for block timestamp
		}
...
History.prototype.getBlockPropagation = function()
{
	...
	// D3.js v3 compatible histogram generation
	var histogram = d3.layout.histogram()
		.range([ MIN_PROPAGATION_RANGE, MAX_PROPAGATION_RANGE ])
		.bins( MAX_BINS )
		( propagation );

	var data = histogram.map(function (val) {
		return {
			x: val.x0,
			dx: val.x1 - val.x0,
			y: val.length,
	...
	var freqCum = 0;
	var histogramData = data.map(function (val) {
		freqCum += val.length;
